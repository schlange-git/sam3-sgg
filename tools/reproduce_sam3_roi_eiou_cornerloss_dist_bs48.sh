...
OUTPUT_DIR="${1:-z_outputs/repro_sam3_roi_eiou_cornerloss_bs48_80000_dist}"
NUM_GPUS="${2:-4}"
...
echo "[Repro SAM3 ROI14 eIoU Corner] GPU=${NUM_GPUS} BS=48 MAX_ITER=80K"
echo "[Repro SAM3 ROI14 eIoU Corner] LR=2e-4 STEPS=(20K,60K) WARMUP=1.5K"
...
TRAIN_OPTS=(
    ...
    # BS=48 equivalent of the previous BS=24 / 160k schedule:
    # same number of samples, half the iterations, and linear LR scaling.
    # GATE_LR_MULTIPLIER is pinned to 1.0 so the newer optimizer rule does not
    # boost roi_refine_head.gate.
    SOLVER.IMS_PER_BATCH "48"
    SOLVER.BASE_LR "0.0002"
    SOLVER.MAX_ITER "80000"
    SOLVER.STEPS "(20000,60000)"
