...
| `overfit_temporal_v2_relation_fixed_all_no_roi_bs12_16000.sh` | 开启全部 temporal relation 修复后的 overfit probe | `z_outputs/overfit_temporal_v2_relation_fixed_all_no_roi_bs48_4000` | `dataset_overfit_temporal`；train/test 都是 `AG_train` | `SAM3=True`，`USE_PATCH_MERGE=False`，`TEMPORAL=True`，独立 relation memory，matched-GT memory update，object gate max=0.20/warmup=500，relation gate max=0.10/warmup=750，`ROI_REFINE=False` | 4 GPU，BS=48，4000 iter，LR=4e-4，steps=(1000,3000)，gate LR x5，eval=500 | 推荐作为修复后第一轮 v2 debug 脚本；样本数等效于单卡 BS12/16000 |
| `overfit_temporal_xsam_roi_fixed_all_bs48_4000.sh` | 将已通过 overfit 的 temporal 修复、X-SAM patch merge、ROI refine 合并验证 | `z_outputs/overfit_temporal_xsam_roi_fixed_all_bs48_4000` | `dataset_overfit_temporal`；train/test 都是 `AG_train` | `SAM3=True`，`USE_PATCH_MERGE=True`，`ROI_REFINE=True`，`SMALL_AREA_THRESH=1.0`，`TEMPORAL=True`，独立 relation memory，matched-GT update，object gate max=0.20/warmup=500，relation gate max=0.10/warmup=750，ROI gate convex fusion | 4 GPU，BS=48，4000 iter，LR=4e-4，steps=(1000,3000)，gate LR x5，eval=500 | 写 `roi_gate_log.csv` 和 `learnable_diagnostics.csv`；后者包含 temporal gate、ROI gate、X-SAM patch merge 偏离 avgpool 初始化、grad/update norm |
...
当前 `overfit_temporal_xsam_roi_fixed_all_bs48_4000` 中三个单独已通过 overfit 的模块同时打开后，训练 loss 继续下降，但 bbox AP/Recall 和 SGG recall 在评测中快速变差。为了判断这是单一 flag 的问题，还是两个模块之间的交互问题，下面三份脚本只改变 `TEMPORAL`、`USE_PATCH_MERGE`、`ROI_REFINE` 三个开关的两两组合，其余数据集、BS、iter、LR、schedule、权重加载和 eval 周期尽量保持一致。

| 脚本 | 打开的两个模块 | 关闭的模块 | 默认输出目录 | 诊断目的 |
|---|---|---|---|---|
| `overfit_temporal_xsam_no_roi_bs48_4000.sh` | fixed temporal v2 relation memory + X-SAM patch merge | ROI refine | `z_outputs/overfit_temporal_xsam_no_roi_bs48_4000` | 判断 X-SAM 与 temporal 同开时是否破坏 object query / bbox 收敛 |
| `overfit_temporal_roi_no_xsam_bs48_4000.sh` | fixed temporal v2 relation memory + ROI refine | X-SAM patch merge | `z_outputs/overfit_temporal_roi_no_xsam_bs48_4000` | 判断 ROI 分类覆盖 / ROI gate 与 temporal memory 同开时是否导致 eval collapse |
| `overfit_xsam_roi_no_temporal_bs48_4000.sh` | X-SAM patch merge + ROI refine | temporal memory | `z_outputs/overfit_xsam_roi_no_temporal_bs48_4000` | 判断 X-SAM 改变主路径特征分布后，是否与 ROI 分类覆盖产生冲突 |
...
| 验证 temporal + X-SAM + ROI 全部打开后的合并 overfit | `overfit_temporal_xsam_roi_fixed_all_bs48_4000.sh` |
| 定位三 flag 中是否存在两两交互问题 | 见“当前问题定位：两两组合 overfit” |
