#!/usr/bin/env bash
# Four-GPU pairwise overfit probe: fixed temporal v2 relation memory + ROI refine.
# X-SAM patch merge is disabled.
...
OUTPUT_DIR="${1:-z_outputs/overfit_temporal_roi_no_xsam_bs48_4000}"
NUM_GPUS="${2:-4}"
...
echo "==========================================================="
echo "[Overfit Temporal + ROI No X-SAM] OUTPUT=${OUTPUT_DIR}"
echo "[Overfit Temporal + ROI No X-SAM] GPU=${NUM_GPUS}, BS=48, MAX_ITER=4000"
echo "[Overfit Temporal + ROI No X-SAM] LR=4e-4 STEPS=(1000,3000) WARMUP=125"
