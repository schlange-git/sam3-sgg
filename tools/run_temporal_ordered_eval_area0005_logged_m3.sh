...
OUT_DIR="/home/cfs/shizekun1_v/sam3-sgg-Auxiliary-Matching/z_outputs/temporal_ordered_eval_area0005_logged_0079999"
...
mkdir -p "${OUT_DIR}"
ROI_EVAL_AREA_THRESH=0.005 PYTHONUNBUFFERED=1 python3 tools/eval_temporal_ordered_full.py \
  --config-file "${CONFIG}" \
  ...
  MODEL.WEIGHTS "${WEIGHTS}" \
  MODEL.ROI_REFINE.EVAL_DUAL False \
  2>&1 | tee "${OUT_DIR}/ordered_eval_area0005_logged.log"
