#!/usr/bin/env bash
# Overfit triplet memory v3: SAM3 + temporal (object_query_memory_v1) + triplet memory
# Single GPU, 1000-frame overfit dataset
set -euo pipefail
PROJ=/home/cfs/shizekun1_v/sam3-sgg-Auxiliary-Matching
cd "$PROJ"
OUTPUT_DIR="${1:-z_outputs/overfit_triplet_memory_v3}"
mkdir -p "${OUTPUT_DIR}"
export ROI_GATE_LOG_PATH="${OUTPUT_DIR}/roi_gate_log.csv"
echo "[TripletMemory v3 Overfit] OUTPUT=${OUTPUT_DIR}"

python3 train_iterative_model.py \
    --num-gpus 1 \
    --config-file configs/speaq_actiongenome_minimal.yaml \
    --dist-url tcp://127.0.0.1:29800 \
...
RET=$?
echo "[TripletMemory v3 Overfit] Done exit=${RET}"
